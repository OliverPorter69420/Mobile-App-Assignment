...
sealed interface SavedLibraryUiState {
    data class Success(val libraries: List<Libraries>?, val libraryBooks: List<LibraryBooks>?) : SavedLibraryUiState
    ...
    fun getLibraries() {
        viewModelScope.launch(Dispatchers.IO) {
            savedLibraryUiState = try {
                val libraries = savedLibrariesRepository.getLibrariesInSavedLibrary()
                SavedLibraryUiState.Success(libraries = libraries , libraryBooks = null)
            } catch (e: Exception) {
    ...
    fun getLibraryBooks(libraryId : Int) {
        viewModelScope.launch(Dispatchers.IO) {
            savedLibraryUiState = try {
                ...
                SavedLibraryUiState.Success(libraries = null , libraryBooks = libraryBooks)
            } catch (e: Exception) {
                SavedLibraryUiState.Error
            }
        }
